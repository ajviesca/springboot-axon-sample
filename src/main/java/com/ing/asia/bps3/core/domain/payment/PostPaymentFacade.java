...
public class PostPaymentFacade {
    ...
    public PostPaymentFacade(BillerRepository billerRepository, PaymentRepository paymentRepository, AccountRepository accountRepository) {
    ...
    public Payment postPayment(PostPaymentCommand command) {
        ...
        BasePaymentEvent debitSourceAccountResultEvent = debitSourceAccount(new DebitSourceAccountCommand(paymentId,
                accountId, billerId, paymentAmount));

        if (isInsufficientBalance(debitSourceAccountResultEvent)) {
            return markPaymentAs(PaymentStatus.FAILED_INSUFFICIENT_BALANCE, paymentId, accountId, billerId, paymentAmount);
        }
        ...
        if (isPaymentUnsuccesful(sendPaymentToBillerReultEvent)) {
            new ReversePaymentExecutor(accountRepository, new ReversePaymentCommand(paymentId, accountId, billerId, paymentAmount)).execute();
            return markPaymentAs(PaymentStatus.FAILED_AND_REVERSED, paymentId, accountId, billerId, paymentAmount);
        }

        return markPaymentAs(PaymentStatus.COMPLETED, paymentId, accountId, billerId, paymentAmount);
    }
    ...
    private BasePaymentEvent sendPaymentToBiller(Long paymentId, Long accountId, Long billerId, BigDecimal paymentAmount) {
        return new SendPaymentToBillerExecutor(billerRepository, new SendPaymentToBillerCommand(paymentId, accountId, billerId,
                paymentAmount)).execute().getResultEvent();
    }
    ...
    private Payment markPaymentAs(PaymentStatus paymentStatus, Long paymentId, Long accountId, Long billerId, BigDecimal paymentAmount) {
        ...
        new UpdatePaymentStatusExecutor(paymentRepository, updatePaymentStatusCommand).execute();
    ...
    private BasePaymentEvent debitSourceAccount(DebitSourceAccountCommand command) {
        return new DebitSourceAccountExecutor(accountRepository, command).execute().getResultEvent();
    }

    private PaymentInProgressEvent createInitialPaymentRecord(PostPaymentCommand commanad) {
        return new PostPaymentExecutor(billerRepository, paymentRepository, commanad).execute().getResultEvent();
    }
}
